...
class Point:
    ...
    def data(self):
        return self  # f'{self.cords}|{self.radius}'
                ...
                if str(self.sf) == 'sticky':
                    ...
                    if (self.collision == 'top') or (self.collision == 'right') or (self.collision == 'bottom') or (self.collision == 'left'):  # colliding with top/right/bottom/left plane
                        ...
                        if self.angleState or (abs(math.degrees(self.bAngle[2])) < 45):
                            resultV -= self.oldVelocity[0] * cos(self.bAngle[2]) / physicsTime
                            resultV -= self.oldVelocity[1] * sin(self.bAngle[2]) / physicsTime
...
class Joint:
    ...
    def data(self):
        return self  # f'{self.cords}|{self.angle}'
    ...
    def update(self):
        ...
        self.angle = getEulerAngle(game.points[self.pOne].cords, game.points[self.pTwo].cords)
...
f = open('physicsOut.txt', 'w')
for p in game.points:
    f.write(str(p.data()))
    f.write('\n')
f.write('\n')
for j in game.joints:
    f.write(str(j.data()))
    f.write('\n')
f.write('\n')
for c in game.collisionRect:
    f.write(str(c.data()))
    f.write('\n')
f.close()
