...
damping = 7  # global damping constant. to allow some centripetal motion, make small. (default value is 100)
    ...
    def constrain(self):
        if (self.height < self.origLength) or (self.height > self.origLength):
            for u in range(3):
                ...
                self.damping[u] = self.dampingConst * abs((self.diff[u] / self.height) * (self.oldHeight - self.height)) * getSign(game.points[self.pOne].velocity[u] - game.points[self.pTwo].velocity[u]) * refreshRate
...
game.collisionRect.append(CollisionRect((50, 30, 50), [-10, 30, 0], [math.radians(0), 0, math.radians(60)]))  # CANNOT be negative angle or above 90 (make near-zero for an angle of 0)
